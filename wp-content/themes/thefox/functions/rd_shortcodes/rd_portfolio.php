...
if($filter_background_color !== '' ){
$output .= '#random'.$portfolio_rand_class.'_port .'.$filter_type.' {background:'.$filter_background_color.'}';
}
if($filter_text_color !== '' ){
$output .= '#random'.$portfolio_rand_class.'_port .'.$filter_type.' #options a{color:'.$filter_text_color.'}';
}
...
echo '<style type="text/css" >#random'.$portfolio_rand_class.'_port .portfolio_desc{ position:absolute; bottom:10%; width:100%; height:40px; padding:0; border:none!important; background:none!important; text-align:center; z-index:3; opacity:0;}#random'.$portfolio_rand_class.'_port .element:hover .portfolio_desc{opacity:1; bottom:50%;}#random'.$portfolio_rand_class.'_port  .element:hover .img_link{left:4px;  opacity:1;}#random'.$portfolio_rand_class.'_port  .element:hover .post_link{right:4px; opacity:1;}';
	...
	if($desc_cat !== ''){
		echo '<style type="text/css" >#random'.$portfolio_rand_class.'_port .portfolio_desc h3{color:'.$desc_cat.'}</style>';
	}
}
if ( $overlay == 'rd_hover_lily' ||  $overlay == 'rd_hover_goliath' ||  $overlay == 'rd_hover_steve'){

	echo '<style type="text/css" >#random'.$portfolio_rand_class.'_port figcaption h2{color:'.$desc_title.';}#random'.$portfolio_rand_class.'_port figcaption p{color:'.$desc_cat.';}#random'.$portfolio_rand_class.'_port .ico_link{background:'.$overlay_color.'!important;}</style>';
}
if ( $overlay == 'rd_hover_sadie'){

	echo '<style type="text/css" >#random'.$portfolio_rand_class.'_port figcaption h2{color:'.$desc_title.';}#random'.$portfolio_rand_class.'_port figcaption p{color:'.$desc_cat.';}#random'.$portfolio_rand_class.'_port figcaption::before{background: -webkit-linear-gradient(top, rgba(72,76,97,0) 0%, '.$overlay_color.' 75%); background: linear-gradient(to bottom, rgba(72,76,97,0) 0%, '.$overlay_color.' 75%);</style>';
}
if ( $overlay == 'rd_hover_bubba' || $overlay == 'rd_hover_chico' ||  $overlay == 'rd_hover_roxy' ||  $overlay == 'rd_hover_layla'  ){

	echo '<style type="text/css" >#random'.$portfolio_rand_class.'_port .ico_link{background:'.$overlay_color.'!important;}</style>';
}
if ( $overlay == 'rd_hover_trending'){

	echo '<style type="text/css" >#random'.$portfolio_rand_class.'_port .port_overlay{background:'.$overlay_color.'!important;}</style>';
}
...
if ( $port_type == 'port_type_9' ){
	...
	$output = '<style type="text/css" >#random'.$portfolio_rand_class.'_port .element{margin-bottom:30px;}';
...
if ( $port_type == 'port_type_10' ){

	$output = '<style type="text/css" >#random'.$portfolio_rand_class.'_port .element{padding: 20px 20px 15px;}';
...
if($port_thumbnail == 'thumbnail_type_5' || $port_thumbnail == 'thumbnail_type_6' ){
	...
	if ($port_type == "port_type_6") {
		echo '
		<div class="port_item_details"><a href="' . $linkToTheWork . '" target="'.$target.'"><h2>' . get_the_title() . '</h2></a><h3>'.$showterm.'</h3>
	...
	echo '
        ...
        <a '.$target.' href="' . $linkToTheWork . '" target="'.$target.'" class="ico_link">';
...
//Set thumbnail depending on portfolio design
	...
	echo   '
		...
		<a href="' . $linkToTheWork . '" target="'.$target.'">View more</a>
		</figcaption>';
    ...
    echo '';
	echo "</span></a><a href='" .  $linkToTheWork  . "' target='".$target."' class='port_post_link'>";
	...
	echo
		...
		<h2><a href="' . $linkToTheWork . '" target="'.$target.'">' . get_the_title() . '</a></h2>
		<h3>'.$showterm.'</h3>
        </div>
	...
	if ( $port_type == "port_type_9" ) {
		...
		echo '<a class="view-portfolio-item" href="' .  $linkToTheWork  . '" target="'.$target.'">' . __('View More', 'thefoxwp') . '</a></div></div>
		</div>';
	}

	if ( $port_type == "port_type_10" ) {
		echo '<div class="port_item_details"><a href="' .  $linkToTheWork  . '" target="'.$target.'"><h2>' . get_the_title() . '</h2></a></div>';
	}
...
echo '<style type="text/css" >#random'.$portfolio_rand_class.'_port .navigation .pagination span,#random'.$portfolio_rand_class.'_port .navigation .pagination a{border:1px solid '.$nav_border.'; color:'.$nav_color.'; background:'.$nav_bg.';}#random'.$portfolio_rand_class.'_port .navigation .pagination .current,#random'.$portfolio_rand_class.'_port .navigation .pagination span:hover,#random'.$portfolio_rand_class.'_port .navigation .pagination a:hover{ color:'.$nav_hover_color.' !important; background:'.$nav_hover_bg.'; border:1px solid '.$nav_hover_bg.'; }#random'.$portfolio_rand_class.'_port .navigation{border-top:1px solid '.$nav_border.';}#random'.$portfolio_rand_class.'_port .pagination_current_position{color:'.$nav_color.';}</style>';
            ...
            <?php
			 if($port_navigation == 'loadmore_nav'){ ?>
			   /*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!CONFIG!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
				...
				var order = "<?php echo esc_js($port_order); ?>";
				var orderby = "<?php echo esc_js($port_orderby); ?>";
                ...
                function get_portfolio_works (this_obj) {
                    ...
                    $.ajax({
                        ...
                        data: "html_template="+html_template+"&now_open_works="+now_open_works+"&action=get_portfolio_works"+"&works_per_load="+works_per_load+"&order="+order+"&orderby="+orderby+"&thumbnail="+thumbnail+"&target=<?php echo esc_js($target); ?>&tags=<?php echo esc_js($tags); ?>&category=<?php echo esc_js($category); ?>",
