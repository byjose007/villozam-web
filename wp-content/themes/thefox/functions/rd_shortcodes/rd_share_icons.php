...
if (!function_exists('rd_share_sc')) {
...
echo '<style type="text/css" >#rd_'.$id.' {margin-top:'.$mt.'px; margin-bottom:'.$mb.'px;}#rd_'.$id.' a:hover {color:'.$hover_color.'; border:1px solid '.$hover_color.';}';
if ($tooltip !== ''){
...
?>
   ...
   <?php } if($twitter !== ''){ ?>
        <li id="twitter" <?php if($animation !== ''){echo 'class="'.$animation.'"';} ?>> <a  target="_blank" onClick="popup = window.open('http://twitter.com/intent/tweet?text=<?php echo esc_attr($msg); ?> <?php echo esc_attr($url); ?>', 'PopupPage', 'height=450,width=500,scrollbars=yes,resizable=yes'); return false" href="#"><i class="fa fa-twitter"></i></a>
         </li>
