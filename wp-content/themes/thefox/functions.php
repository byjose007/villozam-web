...
if ( ! defined( 'CESIS_VERSION' ) ) {
	define( 'CESIS_VERSION', '3.9.72' );
}
...
require get_template_directory() . '/functions/rd_theme_functions/rd-dashboard.php';
...
// Remove WordPress 5.8 widget block editor.
function thefox_widget_block_editor() {
		global $rd_data;
		if($rd_data['rd_block_editor'] !== "yes"){
    remove_theme_support( 'widgets-block-editor' );
		}
}
add_action( 'after_setup_theme', 'thefox_widget_block_editor' );

// Remove script ver ( score optimization, can be removed if needed )
...
function rd_admin_styles() {
		...
		wp_enqueue_script('thefox-backend', get_template_directory_uri() . '/js/thefox_backend.js'  );
...
wp_enqueue_style('js_composer_front',  RD_DIRECTORY. '/css/thefox_js_composer.css');
}
...
function rd_register_required_plugins() {
	$plugins = array(
		...
		array(
			...
			'version' 				=> '6.7.21', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation' 	=> false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
			'external_url' 			=> '', // If set, overrides default API URL and points to an external URL
		),
	...
	$config = array(
		...
		'parent_slug' 	=> 'admin.php', 				// Default parent menu slu
...
// Thumbnails setting
if(isset($rd_data['rd_tnfly']) && $rd_data['rd_tnfly'] == true){
