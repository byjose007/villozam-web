    ...
    var contactForm = {
        ...
        events: function() {
            ...
            form.on('submit', function(e) {
                ...
                // Si el formulario no pasa las validaciones, no continuar
                if ($form.valid()) {
                    $formBtn.prop('disabled', true).data('text', $formBtn.html()).html($formBtn.data('loading-text') || 'Enviando...');
                    ...
                    $.ajax({
                        ...
                        if (data.response === 'success') {
                            ...
                            $form.find('.contact-form-success').removeClass('d-none').addClass('show');
                            $form.find('.contact-form-error').addClass('d-none');
                            $form[0].reset();
                        } else {
                            // Error - Mostrar mensaje de error
                            $form.find('.contact-form-error').removeClass('d-none').addClass('show');
                            $form.find('.mail-error-message').html(data.message);
                            $form.find('.contact-form-success').addClass('d-none');
                        }
                    }).fail(function(err) {
                        // Error en la petición - Mostrar mensaje de error genérico
                        $form.find('.contact-form-error').removeClass('d-none').addClass('show');
                        $form.find('.mail-error-message').html('Ha ocurrido un error inesperado. Por favor, inténtelo más tarde.');
                        $form.find('.contact-form-success').addClass('d-none');
                    }).always(function() {
                        // Restaurar botón
                        $formBtn.prop('disabled', false).html($formBtn.data('text'));
                    });
                }
            });
        },
